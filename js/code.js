...
function listado_data(contador_actual = 1, actualiza_tabla = false) {
    ...
    if (actualiza_tabla) {
        ...
        for (let x = 1; x < contador_futuro; x++) {
            ...
            <td>
            <button class="fas fa-edit fs-4 me-5 negro" onclick="editar_elemento(${contador_actual})"></button>
            <button class="fas fas fa-trash-alt fs-4"  onclick="borrar_elemento(${contador_actual})"></button>
            </td>
            </tr>
            `
        }
        ...
        <td>
        <button class="fas fa-edit fs-5 me-5 logos_botones" onclick="editar_elemento(${contador_actual})"></button>
        <button class="fas fas fa-trash-alt fs-5 logos_botones"  onclick="borrar_elemento(${contador_actual})"></button>
        </td>
        </tr>
        `
    }
...
function enviar_formulario() { //Guarda los datos del usuario y el textbox, luego envia la informacion.
    ...
    alert("Dentro de las proximas 3 horas nos contactataremos contigo y te mostraremos todas las opciones que tienes para armar tu equipo de ensueño. Recuerda #¡SomosGamers!");
    //Codigo para enviar la informacion:
